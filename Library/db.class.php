			...
			*/
			...
			if(isset($periphHistoryData) && $NbRows>1)
			{
				...
				if (!$FlagLastUpdate)
				{
					...
					//update de la valeur courante
					$log->debug("updating Current Value during values import");
					$periph=array("DevicesRawValue" => $periphHistoryData[0][0], "DevicesRawDate"=> $periphHistoryData[0][1]);
					$this->db->DevicesCurrent()->insert_update(array("DevicesRawApiId" => $PeriphId),$periph,$periph);
			...
			// Update des valeurs courantes
			if (isset($periphHistoryData) && $NbRows<=1)
			{
				$log->debug("updating Current Value without history import");
				$periph=$periphHistoryData[0];
				$periph=array("DevicesRawValue" => $periph['0'], "DevicesRawDate"=> $periph['1']);
				$this->db->DevicesCurrent()->insert_update(array("DevicesRawApiId" => $PeriphId),$periph,$periph);
			}
